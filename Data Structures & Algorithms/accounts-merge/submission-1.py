class UnionFind:
    ...
    def find(self, p: int) -> int:
        while p != self.par[p]:
            ...
            p = self.par[p]                     # this allows for a time complexitry upgrade to  N * inverse ackerman where N == size of the nodes which is almost constant.
...
class Solution:
    def accountsMerge(self, accounts: List[List[str]]) -> List[List[str]]:
        ...
        """
        not ideal for hash maps. 
        However, we could have some addition to the map, to make sure it is unique.

        so, some topological sort needed here. 

        how do we know whhich is the "real" one.

        okay. 

        we can have a defaultdict(set), where. Given some name. we tie it initially to a set of emails. 
        if we see the same name again, we will need to somehow use a diff key, but we first check if the email we are currently looking at,
