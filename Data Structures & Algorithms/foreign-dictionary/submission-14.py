class Solution:
    def foreignDictionary(self, words: List[str]) -> str:
        ...
        for i in range(1, len(words)):
            ...
            for j in range(min(len(word1), len(word2))):
                if word1[j] != word2[j]:
                    ...
                    break
        print(adj)
        # N * M where N is length of words array and M is min length of words
        # examples
        """
        "hrn","hrf","er","enn","rfnn"
        ...
        def dfs(src: str) -> bool:
            ...
            return False
        # O(N) where we search each node once. 
        for word in words:
            for letter in word:
                if letter not in visited:
                    if dfs(letter): return ""
                    
        # O(N) to reverse the list and concatenate it to a string.
        return "".join(res[::-1])

        # overall complexity = 
        # time: O(N * M)
        # space: (U + E) where U is the unique characters, and E is the edges stored in adj 
